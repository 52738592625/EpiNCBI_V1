...
to run this code. For example, after installing Python on a Windows PC, and extracting a zip download of the repo, navigate to the newly extracted folder
...
The program will then prompt the user to enter the NCBI Taxonomy ID of the pathogen they'd like to download, and to press enter. IDs can be looked up for individual pathogens on https://www.ncbi.nlm.nih.gov/datasets/

E. Coli's ID is 562, and Salmonella's is 590.

The code will then begin the download, and conclude with cleaning and saving the output data to the same directory main.py resides. Due to the limited bandwidth of NCBI's databases, it may take several minutes for the data to download.
